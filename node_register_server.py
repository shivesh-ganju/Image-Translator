...
class RegisterationServer(BTPeer):
    ...
    def handle_register(self,peerconn,registration_request):
        ...
        if req in self.requests:
            ...
            self.connectandsend(host,port,"DUPL",cmp,pid=self.myid,waitreply=False)
        else:
            ...
            self.nodes[name] = id
            cmp = json.dumps(create_message(self.myid,node_id,node,req,"REGR"))
            self.connectandsend(host,port,"REGR",cmp,pid=self.myid,waitreply=False)
    ...
    def handle_forward(self,peerconn,forward_request):
        ...
        self.connectandsend(host, port, msg["type"], forward_request, pid=self.myid, waitreply=False)
